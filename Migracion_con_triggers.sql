...
/** LO QUE ES LA VALIDACION DE DATOS ENTRARIA EN LA ENTREGA DEL BI. LO DEJAMOS POR ADELANTADO.**/

/*
CREATE FUNCTION LOS_CAPOS.hay_stock_de_autoparte(@autoparte int) returns bit as
begin
	return (select ap.en_stock from LOS_CAPOS.AUTOPARTES ap where ap.id_autoparte = @autoparte)
end
go

CREATE FUNCTION LOS_CAPOS.hay_stock_de_auto(@auto int) returns bit as
begin
	return (select a.en_stock from LOS_CAPOS.AUTOS a where a.id_auto = @auto)
end
go
...
SELECT * FROM LOS_CAPOS.AUTOS
SELECT * FROM LOS_CAPOS.ITEMS_FACTURAS
select LOS_CAPOS.hay_stock_de_autoparte(1001)
select LOS_CAPOS.hay_stock_de_auto(2)
GO

INSERT INTO LOS_CAPOS.ITEMS_FACTURAS 
 (id_autoparte, id_auto)
VALUES (1001, null)
GO

CREATE PROCEDURE LOS_CAPOS.Restar_Autoparte_De_Stock(@autoparte int) AS
	update LOS_CAPOS.AUTOPARTES
		SET en_stock =(SELECT 0 WHERE LOS_CAPOS.AUTOPARTES.id_autoparte = @autoparte)
GO

CREATE PROCEDURE LOS_CAPOS.Restar_Auto_De_Stock(@auto int) AS
	update LOS_CAPOS.AUTOS
		SET en_stock =(SELECT 0 WHERE LOS_CAPOS.AUTOS.id_auto = @auto)
GO

drop trigger LOS_CAPOS.Validar_Stock_En_Venta
go
CREATE TRIGGER LOS_CAPOS.Validar_Stock_En_Venta On LOS_CAPOS.ITEMS_FACTURAS INSTEAD OF INSERT AS
BEGIN


	BEGIN TRANSACTION agregar_item
	DECLARE @id_item int
	DECLARE @id_autoparte int
	DECLARE @id_auto int
	
	/* (1) DECLARO EL CURSOR */	
	DECLARE vendidos CURSOR FOR (
		SELECT id_item, id_autoparte, id_auto
		FROM inserted
	)
	...
	/* (3) HAGO QUE SELECCIONE LA PRIMER FILA (ARRANCA EN EL AIRE) */
	FETCH NEXT FROM vendidos INTO
		@id_item, @id_autoparte, @id_auto
	...
	WHILE @@FETCH_STATUS = 0
			...
			IF @id_autoparte is not null and @id_auto is null
				IF LOS_CAPOS.hay_stock_de_autoparte(@id_autoparte) = 1
					BEGIN
						COMMIT TRANSACTION agregar_item
						EXEC LOS_CAPOS.Restar_Autoparte_De_Stock @autoparte = @id_autoparte
					END
				ELSE
					BEGIN
						ROLLBACK TRANSACTION agregar_item
						PRINT 'SE INTENTO VENDER AUTOPARTE' + STR(@id_autoparte) + 'QUE NO ESTA EN STOCK'	
					END					
			ELSE
				IF @id_autoparte is null and @id_auto is not null 
					IF LOS_CAPOS.hay_stock_de_auto(@id_auto) = 1
						BEGIN
							COMMIT TRANSACTION agregar_item
							EXEC LOS_CAPOS.Restar_Auto_De_Stock @auto = @id_auto
						END
					ELSE
						BEGIN
							ROLLBACK TRANSACTION agregar_item
							PRINT 'SE INTENTO VENDER AUTO' + STR(@id_auto) + 'QUE NO ESTA EN STOCK'	
						END			
				ROLLBACK TRANSACTION
			/* (5) PASO A LA SIGUIENTE*/
			FETCH NEXT FROM vendidos INTO @id_item, @id_autoparte, @id_auto
	
	/* (6) CIERRO EL CURSOR */
	CLOSE vendidos
	/* (7) DESALOCO EL CURSOR */
	DEALLOCATE vendidos
